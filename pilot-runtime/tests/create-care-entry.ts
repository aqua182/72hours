...
import { createAiScribedEntry, createComment, createPatientInsight, listEntryComments, listEntryVersions, listMyPatientInsights, listMyPatientSummaries, markEvidenceClaimConflicted, publishPatientSummary, revertEntryVersion, setCommentResolution } from "../src/db/collaboration-repository";
...
async function main() {
  ...
  try {
    ...
    const myInsights = await withAuthenticatedPilotActor(testWeb, { subject: patientSubject, issuer: "test", audience: "test" }, listMyPatientInsights);
    assert.deepEqual(myInsights.map(({ patientId, content }) => ({ patientId, content })), [{ patientId: fixture.patientId, content: "I am worried the rash is spreading." }], "patient portal receipts must return only the signed-in patient's own submitted insight");
