...
export async function handleCareNoteEvents(request: Request, patientId: string) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      const send = (event: string, data: Record<string, string>) => { if (!closed) controller.enqueue(encoder.encode(`event: ${event}\ndata: ${JSON.stringify(data)}\n\n`)); };
      const poll = async () => {
        if (closed) return;
        try {
        ...
        } catch { if (!closed) { send("closed", {}); closed = true; if (timer) clearInterval(timer); controller.close(); } }
      };
      send("ready", { patientId });
      await poll();
      if (!closed) timer = setInterval(() => { void poll(); }, 3_000);
    },
