...
import { createAiScribedEntry, createComment, createPatientInsight, listClinicCollaborators, listEntryComments, listEntryVersions, listMyPatientInsights, listMyPatientSummaries, markEvidenceClaimConflicted, publishPatientSummary, revertEntryVersion, setCommentResolution } from "../db/collaboration-repository";
...
export async function handlePatientInsight(request: Request) {
  const actorIdentity = await identity(request); if (!actorIdentity) return Response.json({ error: "UNAUTHENTICATED" }, { status: 401 });
  if (request.method === "GET") {
    try { return Response.json({ insights: await withAuthenticatedPilotActor(pool(), actorIdentity, (client) => listMyPatientInsights(client)) }); } catch (error) { return failure(error); }
  }
