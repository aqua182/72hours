...
export async function getCareNote(client: PoolClient, actor: PilotActor, patientId: string): Promise<CareNote> {
  ...
  const entries = await client.query<{ id: string; type: string; author_role: string; current_version: number; content: string; changed_at: string }>(
    `SELECT e.id, e.type, e.author_role, e.current_version, v.content, v.changed_at::text
     FROM care_entries e
  ...
  const highlights = await client.query<{ id: string; title: string; status: string; importance: number; risk_reason: string; evidence_state: string; source_entry_version_id: string }>(
    `SELECT h.id, h.title, h.status, h.importance, h.risk_reason, c.evidence_state, v.id AS source_entry_version_id
     FROM highlights h
  ...
  return {
    ...
    entries: entries.rows.map((entry) => ({ id: entry.id, type: entry.type, authorRole: entry.author_role, currentVersion: entry.current_version, content: entry.content, changedAt: entry.changed_at })),
    highlights: highlights.rows.map((highlight) => ({ id: highlight.id, title: highlight.title, status: highlight.status, importance: highlight.importance, riskReason: highlight.risk_reason, evidenceState: highlight.evidence_state, sourceEntryVersionId: highlight.source_entry_version_id })),
