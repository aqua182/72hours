...
export async function getEvidenceWorkbench(client: PoolClient, actor: PilotActor, highlightId: string): Promise<EvidenceWorkbench> {
  const result = await client.query<{
    highlight_id: string; claim_id: string; title: string; risk_reason: string; importance: number; highlight_status: string; rule_version: string;
    evidence_state: string; entity_type: string; normalized_value: string; extraction_config_version: string;
    source_entry_id: string; source_entry_version_id: string; span_start: number; span_end: number; source_excerpt: string; source_version: number; source_version_content: string;
  }>(
    `SELECT
       h.id AS highlight_id, c.id AS claim_id, h.title, h.risk_reason, h.importance, h.status AS highlight_status, h.rule_version,
       ...
       substring(v.content FROM c.span_start + 1 FOR c.span_end - c.span_start) AS source_excerpt, v.version AS source_version, v.content AS source_version_content
     FROM highlights h
  ...
  return {
    ...
    sourceVersionContent: row.source_version_content,
  };
}
