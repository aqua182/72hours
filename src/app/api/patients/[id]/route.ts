...
export async function GET(_: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const rawHighlights = internal ? rows<any>("SELECT h.*, c.entry_id as entryId, c.version_id as versionId, c.span_start as spanStart, c.span_end as spanEnd, c.state as evidenceState FROM highlights h JOIN evidence_claims c ON c.id=h.claim_id WHERE h.status != 'rejected' ORDER BY h.importance DESC") : [];
  const highlights = rawHighlights.map((highlight) => ({ ...highlight, riskReason: highlight.risk_reason }));
