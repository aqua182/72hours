...
export async function GET(_: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const tasks = internal ? rows("SELECT t.*, u.name as assigneeName FROM tasks t LEFT JOIN users u ON u.id=t.assignee_id WHERE t.patient_id=? AND t.status='open' ORDER BY t.review_required DESC, t.due_at", [id]) : [];
